      ...
      <div class="card-body">
        <form method="GET" action="{{ route('admin.products.edit', $item->slug) }}" class="row g-2">
          ...
          <div class="col-md-4 d-flex gap-2">
            ...
            <a class="btn btn-outline-light" href="{{ route('admin.products.edit', $item->slug) }}">Сброс</a>
          </div>
        </form>
    ...
    <div class="card">
      <div class="card-body">
        <div class="live-preview">
          ...
          <form id="productEditForm" action="{{ route('admin.products.update', $item->slug) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('patch')
            ...
            <div class="row gy-4">
              ...
              <div class="col-12">
                ...
                <textarea id="basic-default-message"
                          ...
                          name="description"
                          placeholder="{{ __('admin.placeholder_text') }}"
                          style="height: 234px;">{{ old('description', $item->description) }}</textarea>
              </div>
...
(function () {
  ...
  // ключ хранения (чтобы у каждого товара свой набор)
  const STORAGE_KEY = 'product_car_ids:' + productId;

  // стартовое состояние: из localStorage ИЛИ из БД
  ...
  function updateUI() {
    // выставить чекбоксы на странице
  ...
  function init() {
    ...
    // перед submit гарантируем актуальный hidden input
    const form = document.getElementById('productEditForm');
    if (form) {
      form.addEventListener('submit', () => {
        // ещё раз обновим hidden
        const hidden = document.getElementById('car_ids_json');
        if (hidden) hidden.value = JSON.stringify(Array.from(selected.values()));
      });
    }
