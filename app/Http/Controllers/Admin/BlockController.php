...
class BlockController extends BaseController
{
  ...
  public function update(UpdateRequest $request, Block $block)
  {
    ...
    if ($block->key === 'catalog_default_parts') {
      ...
      $orderedProducts = collect($request->input('ordered_products', []))
        ->map(fn($id) => (int) $id)
        ->filter(fn($id) => $id > 0)
        ->unique()
        ->values();

      $finalProducts = $orderedProducts
        ->filter(fn($id) => $selectedProducts->contains($id));

      $missingSelected = $selectedProducts
        ->filter(fn($id) => !$finalProducts->contains($id));

      $finalProducts = $finalProducts
        ->concat($missingSelected)
        ->values()
        ->all();
