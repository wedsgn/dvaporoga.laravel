      ...
      if (!Car::whereSlug(Str::slug(trim($row[5]) . ' ' . trim($row[3]) . ' ' . trim($row[4])))->exists()) :
        if ($slug_car_model == CarModel::whereSlug($slug_car_model)->first()->slug) :
          $car = Car::create([
            ...
            'meta_title' => 'Купить ремонтные детали для ' . trim($row[1]) . ' ' . $title_car_model . ' ' . trim($row[5]) . ' ' . trim($row[3]) . '.',
            ...
            'meta_keywords' => '',
            'og_title' => 'Купить ремонтные детали для ' . trim($row[1]) . ' ' . $title_car_model . ' ' . trim($row[5]) . ' ' . trim($row[3]) . '.',
