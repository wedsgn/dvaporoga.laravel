    ...
    foreach ($collection->skip(1) as $row) :
        ...
        if (!CarMake::whereSlug(Str::slug($row[0]))->exists()) :
          $car_make = CarMake::create([
            ...
            'image' => 'default',
            'image_mob' => 'default',
            'description' => '<p>Ремонтные арки и пороги для автомобилей ' . $row[0] . ' с бесплатной доставкой по России.</p>

        <p>100% повторение оригинала. Изготовлено на современном обрудовании с учетом всех форм и изгибов
            оригинальных автомобилей ' . $row[0] . '. Выберите свою модель и перейдите к заказу.</p>',
          ]);
        endif;
