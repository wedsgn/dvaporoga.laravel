...
(function chooseCarInit() {
  ...
  if (!route) {
    console.warn("[choose-car] route empty");
    return;
  }

  function ensureChoices(el) {
    if (!window.Choices) return null;
    if (el.choices && typeof el.choices.setChoices === "function")
      return el.choices;
  ...
  function destroyModelChoices() {
    try {
      if (modelEl.choices && typeof modelEl.choices.destroy === "function")
        modelEl.choices.destroy();
    } catch (e) {}
    var wrap = modelEl.nextElementSibling;
    if (wrap && wrap.classList && wrap.classList.contains("choices"))
      wrap.remove();
  ...
  function setModelLoading() {
    setModelPlaceholder("Загрузка…");
  }
  function setModelError() {
    setModelPlaceholder("Ошибка загрузки");
  }
