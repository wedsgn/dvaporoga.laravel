(() => {
  ...
  const syncMasks = (root) => {
    root
      .querySelectorAll('input[type="tel"], [data-mask], .js-mask')
      .forEach((input) => {
        ...
        if (input.inputmask) {
          try {
            if (typeof input.inputmask.refreshValue === "function")
              input.inputmask.refreshValue();
            else if (typeof input.inputmask.setValue === "function")
  ...
  products.forEach((item) => {
    ...
    addBtn.addEventListener("click", () => {
      ...
      const price =
        item.querySelector(".product-part__price_num span")?.innerHTML ?? "0";
    ...
    const options = {
      get size_id() {
        return sizeSelector.value;
      },
      get steel_type_id() {
        return steelSelector.value;
      },
      get thickness_id() {
        return thicknessSelector.value;
      },
      get type_id() {
